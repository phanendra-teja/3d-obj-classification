...
## 6. PointNet++ (upgrade for Screw's weak accuracy)

Plain PointNet (Steps 3-5 above) extracts one global feature for the
whole point cloud, with no awareness of local neighborhoods. This is
exactly why Screw underperformed in testing: its geometry is small and
scattered, which needs local context to distinguish well -- a single
global summary of the whole shape tends to wash that out.

`model_pointnet2.py` implements a proper PointNet++ (Set
Abstraction/Feature Propagation hierarchy, single-scale grouping, 3
levels), built on `pointnet2_utils.py` (farthest point sampling, ball
query grouping -- pure PyTorch, no custom CUDA build needed). Both were
tested for correct shapes, gradient flow, and an actual training run
with decreasing loss before being handed to you.

Use it via the same `train.py`, just add `--architecture pointnet2`:

```bash
python train.py --dataset points_dataset_clean.npz --epochs 100 --batch_size 4 \
    --points_per_sample 1024 --architecture pointnet2
```

**Expect this to run noticeably slower per epoch than plain PointNet** --
the hierarchical grouping/sampling operations are more expensive than
plain PointNet's simple per-point MLP. On CPU this is quite slow; you
have CUDA available, which will help a lot, but still expect longer
wall-clock time than the plain PointNet runs you've already done.

`predict.py` automatically detects which architecture a checkpoint was
trained with (saved in the checkpoint file) and loads the right model
class -- no extra flag needed at inference time.

**Radius values in `model_pointnet2.py`** (0.2 and 0.4 for the two local
SA levels) assume point clouds normalized to a unit sphere, which is
exactly what `prepare_dataset.py`'s normalization does -- if you ever
change that normalization, these radii need retuning to match, or the
local neighborhoods will end up too large or too small to be meaningful.
